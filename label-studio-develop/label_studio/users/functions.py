...
'''
def VIT_super_edit_userpermission(request,user_form, permission_form, next_page):
    """Save super user instance to DB"""
    adduser=users.models.User.object.get(email=str(user_form.email))
    addorg=Organization.objects.get(title='TempOrganization')
    delorg=Organization.objects.get(title=str(adduser.active_organization.title))
...
def VIT_super_proceed_permissionedit(request,user_form, permission_form, next_page):
    """Register a new user for POST user_signup"""
    # save user to db
    response = VIT_super_edit_userpermission(request,user_form, permission_form, next_page)
    return response
