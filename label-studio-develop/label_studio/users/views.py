...
from organizations.forms import OrganizationSignupForm,VIT_OrganizationForm
from organizations.models import Organization
from organizations.functions import create_organization,destroy_organization
from rest_framework.authtoken.models import Token
from users import forms
from users.models import User
from users.functions import login, proceed_registration,VIT_super_proceed_registration,VIT_base_proceed_registration,VIT_super_proceed_useraddorg,VIT_super_proceed_userdelorg
...
'''
@login_required
def VIT_super_user_editpermission(request):
    user = request.user
    if not user.is_superuser:
        raise PermissionDenied()
    next_page = request.GET.get('next')
    ...
    # make a new user
    user_form = forms.VIT_super_UserForm()
    permission_form=forms.VIT_permissionForm()
    if request.method == 'POST':
        user_form=forms.VIT_super_UserForm(request.post)
        permission_form.VIT_permissionForm(request.post)
        if user_form.is_valid() and permission_form.is_valid():
            redirect_response = VIT_super_proceed_permissionedit(request, user_form, permission_form, next_page)
            if  redirect_response:
                return redirect_response
    return render(
        request,
