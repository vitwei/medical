...
class VIT_super_UserSignupForm(forms.Form):
    email = forms.EmailField(label='Work Email', error_messages={'required': 'Invalid email'})
    password = forms.CharField(
        max_length=PASS_MAX_LENGTH,
        error_messages={'required': PASS_LENGTH_ERR},
    ...
    def save(self):
        ...
        if 'allow_newsletters' in cleaned:
            allow_newsletters = cleaned['allow_newsletters']
        user = User.objects.create_superuser(email, password, allow_newsletters=allow_newsletters)
        return user
    
    
class VIT_super_UserUpdateForm(forms.ModelForm):
    class Meta:
        model = User
        fields="__all__"
    def clean_email(self):
...
class VIT_super_Form(forms.ModelForm):
    class Meta:
        model = User
        fields="__all__"
    '''
