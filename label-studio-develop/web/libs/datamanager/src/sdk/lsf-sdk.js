...
export class LSFWrapper {
  ...
  onSubmitAnnotation = async () => {
    ...
    if (status === 200 || status === 201) this.datamanager.invoke("toast", { message: "标注创建成功", type: "info" });
  ...
  onUpdateAnnotation = async (ls, annotation, extraData) => {
    ...
    if (status === 200 || status === 201) this.datamanager.invoke("toast", { message: "标注更新成功", type: "info" });
  ...
  /** @private */
  onCensorAnnotation = async (ls, annotation, extraData) => {
    const { task } = this;
  ...
  /** @private */
  CensorData(annotation,{ includeId, draft } = {}) {
    const userGenerate =
